...
function Nav() {
  let [showHam, setShowHam] = useState(false); // state for hamberger for small screen size
  let [showPro, setShowPro] = useState(false); // state to show profile on NavBar
  let navigate = useNavigate();
  let dispatch = useDispatch();
  let { userData } = useSelector((state) => state.user); // get data from Redux UserSlice
  ...
  const handleLogout = async () => {
    try {
      ...
      await dispatch(setUserData(null)); // set data to null on store userSlice
    ...
    <div>
      <div className="w-[100%] h-[70px] fixed top-0 px-[20px] py-[10px] flex items-center justify-between bg-[#00000047]  z-10">
        ...
        <div className="w-[30%] lg:flex items-center justify-center gap-4 hidden">
          ...
          {/* If data not found  login Button  */}
          {!userData && (
            <span
          ...
          {/* Id user data is present then Logout Button */}
          {userData && (
            <span
        ...
        {/* if show profile true then show My profile and my courses */}
        {showPro && (
      ...
      {/* Hamburger for smalll size screens */}
        <GiHamburgerMenu
          className="w-[30px] h-[30px] lg:hidden fill-white cursor-pointer "
          onClick={() => setShowHam((prev) => !prev)}
        />
      </div>
      {/* for small size screen Hamburger NavBar extension */}
      <div
        className={`fixed top-0 w-[100vw] h-[100vh] bg-[#000000d6] flex items-center justify-center flex-col gap-5 z-10 ${showHam ? "translate-x-[0%] transition duration-600  ease-in-out" : "translate-x-[-100%] transition duration-600  ease-in-out"}`}
      >
        <GiSplitCross
